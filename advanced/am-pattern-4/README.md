# Helm Chart for deployment of WSO2 API Manager with a Internal and External Gateway and separate Key Manager along with WSO2 API Manager Analytics
...
#### Install Chart From [WSO2 Helm Chart Repository](https://hub.helm.sh/charts/wso2)
 ...
 ```
 helm install --name <RELEASE_NAME> wso2/am-pattern-4 --version 3.1.0-4 --namespace <NAMESPACE>
 ```
 ...
 - Deploy the Kubernetes resources using the Helm Chart
    ...
    ```
    helm install <RELEASE_NAME> wso2/am-pattern-4 --version 3.1.0-4 --namespace <NAMESPACE>
    ```
...
```
 helm install --name <RELEASE_NAME> wso2/am-pattern-4 --version 3.1.0-4 --namespace <NAMESPACE> --set wso2.subscription.username=<SUBSCRIPTION_USERNAME> --set wso2.subscription.password=<SUBSCRIPTION_PASSWORD>
```
...
##### Deploy Helm chart for WSO2 API Manager Pattern 4 deployment.
 ...
 ```
 helm install --dep-up --name <RELEASE_NAME> <HELM_HOME>/am-pattern-4 --version 3.1.0-4 --namespace <NAMESPACE>
 ```
 ...
 - Deploy the Kubernetes resources using the Helm Chart
    ...
    ```
    helm install <RELEASE_NAME> <HELM_HOME>/am-pattern-4 --version 3.1.0-4 --namespace <NAMESPACE> --dependency-update
    ```
...
```
 helm install --name <RELEASE_NAME> <HELM_HOME>/am-pattern-4 --version 3.1.0-4 --namespace <NAMESPACE> --set wso2.subscription.username=<SUBSCRIPTION_USERNAME> --set wso2.subscription.password=<SUBSCRIPTION_PASSWORD>
```
...
- NAME: Metadata names of the Kubernetes Ingress resource (defaults to `wso2am-pattern-4-am-gateway-external-ingress & wso2am-pattern-4-am-gateway-internal-ingress`)
- HOSTS: Hostnames of the WSO2 API Manager's External and Internal Gateway services (`<wso2.deployment.am.gateway.external.hostname> & <wso2.deployment.am.gateway.internal.hostname>`)
- ADDRESS: External IP (`EXTERNAL-IP`) exposing the API Manager's Gateway services to outside of the Kubernetes environment
...
| `wso2.deployment.am.gateway.external.ingress.annotations`                   | Annotations for the External Gateway Services Ingress                                     | `Nginx ingress annotations`       |
| `wso2.deployment.am.gateway.internal.hostname`                              | Hostname for API Manager Gateway                                                          | `internal.gateway.am.wso2.com`       |
| `wso2.deployment.am.gateway.internal.ingress.annotations`                   | Annotations for the Internal Gateway Services Ingress                                     | `Nginx ingress annotations`       |
